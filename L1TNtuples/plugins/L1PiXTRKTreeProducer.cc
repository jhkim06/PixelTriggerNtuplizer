  ...
  simTrack_ (iConfig.getParameter<edm::InputTag> ("simTrack"))
{
  ...
  tree_->Branch("cl3d_bdteg", &cl3d_bdteg_);
  tree_->Branch("cl3d_egid", &cl3d_egid_);
...
L1PiXTRKTreeProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(size_t i = 0; i < genParticles->size(); ++ i) {
    ...
    std::cout << i << " th ptl: " << " id: " << p.pdgId() << " charge: " << p.charge() << " pt: " << p.pt() << " eta: " << p.eta() << std::endl;
  ...
  if(electronTracks.size() != 0){
      ...
      const reco::GenParticle & p = (*genParticles)[1];
...
/*
  ...
  RawParticle electron(genParticle[1].p4());
  //RawParticle electron(genParticle[eleIndx_last].p4());
  electron.setVertex(genParticle[1].vertex().x(), genParticle[1].vertex().y(), genParticle[1].vertex().z(), 0.);
  ...
  {
    // something failed
    trueElectron = genParticle[1].polarP4();
  ...
  //std::cout << "pt: " << genParticle[1].pt() << " eta: " << genParticle[1].eta() << std::endl;
  //std::cout << "initial r: " << sqrt(pow(genParticle[1].vertex().x(),2.) + pow(genParticle[1].vertex().y(),2.)) << " z: " << genParticle[1].vertex().z() << std::endl;

  for( int i = 0; i < 4; i++){
     RawParticle electron_test(genParticle[1].p4());
     electron_test.setVertex(genParticle[1].vertex().x(), genParticle[1].vertex().y(), genParticle[1].vertex().z(), 0.);
...
*/
  ...
  //{
  //  // something failed
  //  //truePositron = genParticle[1].polarP4();
  ...
  for(auto cl3d_itr=multiclusters.begin(0); cl3d_itr!=multiclusters.end(0); cl3d_itr++)
  {
    ...
    cl3d_egid_.emplace_back(id_->decision(*cl3d_itr));
